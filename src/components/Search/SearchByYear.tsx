...
export const SearchByYear = (): JSX.Element => {
  ...
  const yearSortByNumber = data?.Page?.media?.sort(
    (a, b) => (b?.startDate?.year ?? 0) - (a?.startDate?.year ?? 0),
  );
  const yearArray = yearSortByNumber?.map(
    (years) => years?.startDate?.year?.toString(),
  );

  const yearByInput = doubleDelete(yearArray)?.filter(
    (mediaYear) => mediaYear?.slice(0, searchYear.length) === searchYear,
  );
  ...
  return (
    <div>
      <SearchSectionName>Year</SearchSectionName>
      <SearchSection
        onClick={() => {
          setYearList(true);
        }}
